...
- `context`: return Memory-produced `context_text` plus bounded structured `evidence` when the app wants context for its own answerer, tools, actions, evidence views, or debugging. Memory always returns both surfaces; there is no request-time format selector.
