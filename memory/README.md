...
PUT  /v1/principal-memberships/{group_type}/{group_id}
...
アプリ側の channel、DM、project、folder などで参加者が変わった場合、アプリは対応する owner principal の principal membership を更新します。参加者一覧を authoritative に持っている場合は `PUT` で同期し、単発の join / leave event だけがある場合は `POST` / `DELETE` を使います。
...
- `budget.estimated_tokens`: Memory が見積もった response payload の token 数。app 側の prompt 組み立てで使えます。
...
`evidence.memory_atoms[].role` は `direct` / `related` など、アプリが evidence の扱いを判断するための最低限の順序・役割情報です。event time normalization、raw evidence selection mode、included character count、target token budget、debug ranking、omitted reason、policy trace、warnings は external response には含めません。
