# How to Use Memory from an App

This guide is the practical call sequence for an application backend integrating with Memory. It complements the boundary contract in `chat-memory-boundary.md`.
...
Production app integrations authenticate as the application and delegate each user/team request to a canonical Memory principal.

Every user, DM, channel, team, or workspace read/write request should include:
...
Send the payload shape shown in `examples/chat-bootstrap.json`.
